...
class ZyczeniaOgolneInline(OgraniczPionyMixin, SortableTabularInline):
...
class PriorytetInline(SortableTabularInline):
    model = Priorytet
...
class ZyczeniaPracownikaAdmin(SortableAdmin):
